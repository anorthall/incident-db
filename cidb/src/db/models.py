...
from cidb.src.db.choices import Countries, USStates


def publication_upload_path(instance: Publication, filename: str) -> str:
    ...
    class Meta:
        constraints = (
            models.CheckConstraint(
                condition=(Q(state="") & ~Q(us_state=""))
                | (~Q(state="") & Q(us_state=""))
                | (Q(state="") & Q(us_state="")),
                name="incident_state_xor_us_state",
            ),
            models.CheckConstraint(
                condition=(Q(country="US") & Q(state="")) | (~Q(country="US")),
